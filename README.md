# Enumer [![Go Reference](https://pkg.go.dev/badge/github.com/mihaiav/enumer.svg)](https://pkg.go.dev/github.com/mihaiav/enumer) [![Go Report Card](https://goreportcard.com/badge/github.com/mihaiav/enumer)](https://goreportcard.com/report/github.com/mihaiav/enumer)
...
```
go install github.com/mihaiav/enumer@latest
```
...
be almost meaningless or hard to trace or use by a human.
  ...
  * Method `Enum()`: returns the typed enum values for schema/introspection consumers.
...
* When `datastore` is enabled, `Save()`, `Load(any)`, and `Zero()` are generated. Together they expose the persisted string form and its representation type.
* When `query` is enabled, encoder and decoder functions are registered with `x.do.at/encoding/query`.
...
To transform it to lowerCamel, snake_case, or kebab-case, use the `transform` flag.
...
```
`lowercamel` preserves all-uppercase acronyms while lowercasing the first rune of other names. This matches x.do.at's historical enum wire-name convention.

## How to use
The usage of Enumer is the same as Stringer, so you can refer to the [Stringer docs](https://godoc.org/golang.org/x/tools/cmd/stringer)
for more information.

The `json`, `text`, `datastore`, and `query` flags default to true. The `yaml` and `sql` flags default to false. You can explicitly enable or disable any of them, for example `enumer -type=Pill -query=false -json=true`.

Use `-tests` when the requested enum type is declared in a `_test.go` file. Enumer loads Go's test package variants and selects the one that actually defines every requested type.


For enum string representation transformation the `transform` and `trimprefix` flags
were added (i.e. `enumer -type=MyType -json -transform=snake`).
Possible transform values are `lowercamel`, `snake`, and `kebab`.
...
## Generated string metadata convention

For each enum type, Enumer emits a literal string slice named `_<Type>StringValues`, in numeric enum order after duplicate removal and after all prefix, transform, and line-comment processing:

```go
var _PillStringValues = []string{"placebo", "aspirin", "ibuprofen"}
```

The declaration is intentionally simple and stable so source-analysis tools can read the exact wire values without evaluating the compact Stringer lookup tables or guessing how names were transformed. It is generated metadata; application code should continue to use `String()`, `<Type>String`, `<Type>Values`, or `Enum()`.
