...
var (
	...
	tests     = flag.Bool("tests", false, "if true, include _test.go files and select the test package defining the requested type. Default: false")

	output          = flag.String("output", "", "output file name; default srcdir/<type>_enumer.go")
	transformMethod = flag.String("transform", "noop", "enum item name transformation method (noop, lowercamel, snake, kebab). Default: noop")
...
func main() {
	...
	g.parsePackageForTypes(args, types, *tests)
	...
	tmpFile, err := os.CreateTemp(filepath.Dir(outputName), tmpName)
	if err != nil {
	...
	if err = tmpFile.Chmod(0644); err != nil {
		tmpFile.Close()
		os.Remove(tmpPath)
		log.Fatalf("setting output permissions: %s", err)
	}
	...
	err = os.Rename(tmpPath, outputName)
	if err != nil {
		os.Remove(tmpPath)
...
func (g *Generator) parsePackage(patterns []string) {
	g.parsePackageForTypes(patterns, nil, false)
}

func (g *Generator) parsePackageForTypes(patterns, requestedTypes []string, includeTests bool) {
	cfg := &packages.Config{
	...
	if !includeTests || len(requestedTypes) == 0 {
		log.Fatalf("error: %d packages found", len(pkgs))
	}

	var matches []*packages.Package
	for _, pkg := range pkgs {
		if packageDefinesTypes(pkg, requestedTypes) {
			matches = append(matches, pkg)
		}
	}
	if len(matches) != 1 {
		log.Fatalf("error: expected one package defining %q, found %d among %d packages", requestedTypes, len(matches), len(pkgs))
	}
	g.addPackage(matches[0])
}

func packageDefinesTypes(pkg *packages.Package, requestedTypes []string) bool {
	...
	for _, file := range pkg.Syntax {
		for _, decl := range file.Decls {
			...
			for _, spec := range gen.Specs {
				found[spec.(*ast.TypeSpec).Name.Name] = true
			}
		}
	}
...
func (g *Generator) transformValueNames(values []Value, transformMethod string) {
	if transformMethod == "lowercamel" {
		for i := range values {
			values[i].name = lowerCamel(values[i].name)
		}
		return
	}
...
// lowerCamel matches x.do.at's historical enum wire-name convention: lower
// the first rune unless the whole name is an uppercase acronym.
